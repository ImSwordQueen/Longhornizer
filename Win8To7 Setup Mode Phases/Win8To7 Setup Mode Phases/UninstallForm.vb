...
#End Region
    ...
    Private Sub UninstallForm_FormClosing(sender As System.Object, e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        If PageTransform.Visible = True Then
            MsgBox("This operation cannot be cancelled, as cancelling this operation can risk rendering Windows unusable.", MsgBoxStyle.Exclamation, "Longhornizer Transformation Pack ")
            e.Cancel = True
        End If
    End Sub
...
#Region "Buttons"
    ...
    Private Sub NextConfirmRemoval_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles NextConfirmRemoval.Click
        ...
        Try
            IO.File.Copy(storagelocation + "\SetupTools\setup.exe", windir + "\Temp\Longhornizeruninstall.exe", True)
...
#Region "Start Restoration"
    Sub DoTheJobRestore()
        ...
        Shell(windir + "\" + sysprefix + "\cmd.exe /c wmic.exe /Namespace:\\root\default Path SystemRestore Call CreateRestorePoint ""Before uninstalling Longhornizer Transformation Pack "", 100, 12", AppWinStyle.Hide, True)
    ...
    Sub EnterRestoration()
        ...
        HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("CurrentPhase", 97)
...
#Region "Start Removal"
    Sub DoTheJobRemove()
        ...
        ChangeProgressStyle(ProgressBarStyle.Marquee)
        Shell(windir + "\" + sysprefix + "\cmd.exe /c reg add ""HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore"" /v ""SystemRestorePointCreationFrequency"" /t REG_DWORD /d 0 /f", AppWinStyle.Hide, True)
        Shell(windir + "\" + sysprefix + "\cmd.exe /c wmic.exe /Namespace:\\root\default Path SystemRestore Call CreateRestorePoint ""Before deleting Longhornizer Transformation Pack  backups"", 100, 12", AppWinStyle.Hide, True)
        Shell(windir + "\" + sysprefix + "\cmd.exe /c reg del ""HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore"" /v ""SystemRestorePointCreationFrequency"" /f", AppWinStyle.Hide, True)
        ChangeProgressStyle(ProgressBarStyle.Continuous)
    ...
    Sub EnterRemoval()
        ...
        HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("CurrentPhase", 69)

        'Switch to progress form
        Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SYSTEM\Setup"" /v CmdLine /d " + windir + "\Temp\Longhornizeruninstall.exe" + " /t REG_SZ /f", AppWinStyle.Hide, True)
