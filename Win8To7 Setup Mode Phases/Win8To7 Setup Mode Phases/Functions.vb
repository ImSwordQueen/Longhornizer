...
#Region "Variables"
    ...
    Public storagelocation As String = windir + "\Longhornizer"
    Public storagelocationuser = "\AppData\Local\Longhornizer"
...
#Region "Init and End"
    ...
    Function getRandomTEMPfolder()
        Dim result As String = windir + "\TEMP\Longhornizer-" + rng.Next(0, 999999999 + 1).ToString()
        While IO.File.Exists(result)
            result = windir + "\TEMP\Longhornizer-" + rng.Next(0, 999999999 + 1).ToString()
...
#Region "File and Directory Manipulation"
    Function MoveFile(ByVal originalpath As String, ByVal newpath As String, ByVal jobDescription As String, Optional ByVal copyFile As Boolean = False)
        'e.g.: C:\Windows\System32\imageres.dll, C:\Windows\Longhornizer\Backups\Windows\System32\imageres.dll, "file patching - to backups", True
    ...
    Function MoveFolder(ByVal originalpath As String, ByVal newpath As String, ByVal jobDescription As String, Optional ByVal copyFolder As Boolean = False)
...
#Region "Resource/File Extraction"
    Function ExtractEverything()
        ...
        For Each direc In {storagelocation + "\SetupFiles", storagelocation + "\SetupTools", windir + "\" + sysprefix + "\OldNewExplorer", windir + "\" + sysprefix + "\Longhornizer"}
            Try
                CreateDir(direc)
            Catch ex As Exception
        ...
        For Each dictEntry In My.Resources.ResourceManager.GetResourceSet(System.Globalization.CultureInfo.CurrentCulture, True, True) 'Iterate over our Resources:
            ...
            If targetPath.StartsWith("SMOpenShell_") And Not HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("Start") = "openshell" Then '  Skip Open Shell if not chosen
                Continue For
            End If

            ' Now skip depending on remaining preferences
            If targetPath.StartsWith("AllowUXThemePatcher_") And Not HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).GetValue("AllowUXThemePatcher") = "true" Then '  Skip UXThemePatcher stuff if not chosen
                Continue For
            End If
            ...
            If targetPath.StartsWith("setupbgproc:") Then
                ...
                If IO.File.Exists(windir + "\" + sysprefix + "\Longhornizer\" + targetPath) Then
                    Continue For
                End If

                'Now copy the new file into the temporary path
                If WriteFileFromResources(dictEntry.Key.ToString(), windir + "\" + sysprefix + "\Longhornizer\" + targetPath) = False Then
                    Thread.Sleep(10000)
