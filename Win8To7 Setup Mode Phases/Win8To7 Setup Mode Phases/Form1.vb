...
#End Region
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'High Contrast check
        If HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("HighContrast") = 1 Then 'Do a HC check manually as ThemeChanged only triggers if the theme changes
            RefreshHC(True)
        Else
            RefreshHC()
        End If
        ...
        If HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("CurrentPhase") = 2 Then 'Load the code for the appropriate phase, in a separate thread
            ...
            jobthread.Start()
        ElseIf HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("CurrentPhase") = 3 Then
            Dim jobthread As New Thread(AddressOf Phase3)
            jobthread.Start()
        ElseIf HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("CurrentPhase") = 4 Then
            Dim jobthread As New Thread(AddressOf Phase4)
            jobthread.Start()
        ElseIf HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("CurrentPhase") = 103 Then 'Stage 103 = Stage 2, with slight differences
            Dim jobthread As New Thread(AddressOf Phase2)
            jobthread.Start()
        ElseIf HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("CurrentPhase") = 104 Then 'Stage 104 = Stage 4, with slight differences
            Dim jobthread As New Thread(AddressOf Phase4)
            jobthread.Start()
        Else
            End
...
#Region "Important calls"
    Private Sub ErrorOccurred(ByVal status As String)
        ...
        HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("TransformationFailed", 1)
    ...
    Sub Phase2()
        Try
            ...
            If forCustomise = False Then
                ...
                HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("CurrentPhase", 3)
                Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SYSTEM\Setup"" /v CmdLine /d " + storagelocation + "\SetupTools\setup.exe" + " /t REG_SZ /f", AppWinStyle.Hide, True)
                Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SYSTEM\Setup"" /v OOBEInProgress /t REG_DWORD /d 1 /f", AppWinStyle.Hide, True)
                Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SYSTEM\Setup"" /v SetupPhase /t REG_DWORD /d 4 /f", AppWinStyle.Hide, True)
                Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SYSTEM\Setup"" /v SetupType /t REG_DWORD /d 2 /f", AppWinStyle.Hide, True)
            ...
            Else
                ...
                HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("CurrentPhase", 104)
            End If
...
#Region "Stage 3 of 4"
    Sub Phase3()
        Try
            ...
            If HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer").GetValue("AllowUXThemePatcher") = "true" And IO.File.Exists(storagelocation + "\SetupFiles\UXThemePatcher.exe") Then
                'Install UXThemePatcher
                Shell(storagelocation + "\SetupFiles\UXThemePatcher.exe /S", AppWinStyle.NormalFocus, True, 2400000)
                '  NOTE: Newer versions of UXThemePatcher no longer respect /S - if you need a modern version of UXThemePatcher, good luck I guess.
            ...
            HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("CurrentPhase", 4)
            ChangeProgress(75)
            ChangeProgress(100)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SYSTEM\Setup"" /v CmdLine /d " + storagelocation + "\SetupTools\setup.exe" + " /t REG_SZ /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SYSTEM\Setup"" /v OOBEInProgress /t REG_DWORD /d 1 /f", AppWinStyle.Hide, True)
...
#Region "Stage 4 of 4"
    Sub Phase4()
        Try
            ...
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""DisplayName"" /t REG_SZ /d ""Longhornizer Transformation Pack "" /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""DisplayVersion"" /t REG_SZ /d ""3.1"" /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""DisplayIcon"" /t REG_SZ /d """ + storagelocation + "\SetupTools\setup.exe"" /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""InstallLocation"" /t REG_SZ /d """ + storagelocation + "\SetupTools""\ /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""UninstallString"" /t REG_SZ /d """ + storagelocation + "\SetupTools\setup.exe"" /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""NoModify"" /t REG_DWORD /d 1 /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""NoRepair"" /t REG_SZ /d 1 /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""VersionMajor"" /t REG_SZ /d 3 /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""VersionMinor"" /t REG_SZ /d 2 /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""Publisher"" /t REG_SZ /d ""ImSwordKing and co."" /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""Comments"" /t REG_SZ /d ""Uninstall Longhornizer Transformation Pack , or revert Windows to how it was before transforming"" /f", AppWinStyle.Hide, True)
            Shell(windir + "\" + sysprefix + "\cmd.exe /c reg ADD ""HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\LonghornizerTransformationPack"" /v ""URLInfoAbout"" /t REG_SZ /d ""https://forum.eclectic4un.me/viewtopic.php?f=29&t=107"" /f", AppWinStyle.Hide, True)
            ...
            HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).DeleteValue("HighContrast", False)
            'Save our file changes lists to Registry
            HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("FilesChanged", MultiSZFileChanges.ToArray, Microsoft.Win32.RegistryValueKind.MultiString)
            HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("UserFilesChanged", MultiSZUserFileChanges.ToArray, Microsoft.Win32.RegistryValueKind.MultiString)
            HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("FoldersChanged", MultiSZFolderChanges.ToArray, Microsoft.Win32.RegistryValueKind.MultiString)
            HKLMKey32.OpenSubKey("SOFTWARE\Longhornizer", True).SetValue("UserFoldersChanged", MultiSZUserFolderChanges.ToArray, Microsoft.Win32.RegistryValueKind.MultiString)
